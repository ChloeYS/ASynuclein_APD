...
# # SEX STATISTICS: FISHER'S TEST
# fisher.test(table(df$Sex, df$RTQUIC)) # Expected count is <5 for one cell
...
# # PARK ONSET STATISTICS: FISHER'S TEST
# fisher.test(table(df$Parkinsonian_onset, df$RTQUIC)) # Expected count is <5 for one cell
...
# sum(df$AD=="AD Positive" & df$DX_APD=="CBS" & df$Sex=="M")
# sum(df$AD=="AD Positive" & df$DX_APD=="PSP" & df$Sex=="M")
# df %>% group_by(AD) %>% count(APOEe4)

# cat("\n\n#######################################################################################################\n",
# 	"                          6.1. AD+ COHORT CHARACTERISTICS: CATEGORICAL VARIABLES \n",
# 	   "#######################################################################################################\n")

# cat("----------------------------   GOES IN eTABLE 2: CBS-AD- vs CBS-AD+ --------------------------------------\n")

# # DX STATISTICS: DISTRIBUTION
# table(df$AD, df$DX_APD) 

# # DX+ STATISTICS: CHI SQUARE
# chisq.test(table(df$AD, df$DX_APD), correct=F)
...
# # SEX STATISTICS: FISHER'S TEST
# fisher.test(table(df$Sex, df$AD)) # Expected count is <5 for one cell


# # APOE STATISTICS: DISTRIBUTION
# table(df$APOEe4, df$AD)

# # APOE STATISTICS: FISHER'S TEST
# fisher.test(table(df$APOEe4, df$AD)) # Expected count is <5 for one cell

# # PARK ONSET STATISTICS: DISTRIBUTION
# table(df$Parkinsonian_onset, df$AD)

# # PARK ONSET STATISTICS: FISHER'S TEST
# fisher.test(table(df$Parkinsonian_onset, df$AD)) # Expected count is <5 for one cell



# cat("\n\n#######################################################################################################\n",
# 	"                          6.1. AD+: NUMERICAL VARIABLES \n",
# 	   "#######################################################################################################\n")
...
# shapiro.test(APOEposdf$Onset_age) #normal
# shapiro.test(APOEnegdf$Onset_age) #normal
# leveneTest(Onset_age ~ DX_APD*APOEe4*AD, df) #Homoscedasticity. Specify saturated model, ie includes the interaction term even if aovmodel does not

# # ONSET STATISTICS: SUMMARY
# df %>% group_by(DX_APD) %>% summarize(count=n(), mean=format(round(mean(Onset_age, na.rm=T),3),3), sd=format(round(sd(Onset_age, na.rm=T),3),3))
# df %>% group_by(AD) %>% summarize(count=n(), mean=format(round(mean(Onset_age, na.rm=T),3),3), sd=format(round(sd(Onset_age, na.rm=T),3),3))


# # ONSET STATISTICS: ANCOVA SELECTION
# # Compare models with Ftest. Cannot include APOE as there is missing data. 
# test1 <- lm(Onset_age ~ AD, df)
# test2 <- lm(Onset_age ~ DX_APD, df)
# test3 <- lm(Onset_age ~ DX_APD + AD, df)
# anova(test1, test3)
# anova(test2, test3) #dx_APD by itself is better thna with AD
# dfapoe <- df[!is.na(df$APOEe4), ]
# test1 <- lm(Onset_age ~ APOEe4, dfapoe)
# test2 <- lm(Onset_age ~ DX_APD, dfapoe)
# test3 <- lm(Onset_age ~ DX_APD +APOEe4, dfapoe)
# anova(test1, test3) 
...
# aov <- aov(Onset_age ~ DX_APD*APOEe4 + AD, df) #The model reported as we are interested to describe the effect (here non-effect) of AD
...
# aov2 <- aov(Onset_age ~ DX_APD + AD, df) #The model reported as we are interested to describe the effect (here non-effect) of AD
# summary(aov2)
# Anova(aov2, type="II") 
...
# shapiro.test(ADposdf$Park_onset) #normal
# shapiro.test(ADnegdf$Park_onset) #normal
# shapiro.test(APOEposdf$Park_onset) #normal
# shapiro.test(APOEnegdf$Park_onset) #normal
# leveneTest(Park_onset ~ DX_APD*APOEe4*AD, df) #Homoscedasticity. Specify saturated model, ie includes the interaction term even if aovmodel does not

# # PARK ONSET STATISTICS: SUMMARY
# df %>% group_by(DX_APD) %>% summarize(count=n(), mean=format(round(mean(Park_onset, na.rm=T),3),3), sd=format(round(sd(Park_onset, na.rm=T),3),3))
# df %>% group_by(AD) %>% summarize(count=n(), mean=format(round(mean(Park_onset, na.rm=T),3),3), sd=format(round(sd(Park_onset, na.rm=T),3),3))

# # PARK ONSET STATISTICS: ANCOVA
# aov2 <- aov(Park_onset ~ DX_APD + AD, df) #The model reported as we are interested to describe the effect (here non-effect) of AD
# summary(aov2)
# Anova(aov2, type="II") 
...
# #AD+: FISHER
# 	# chisq.test(table(df$AD, df$RTQUIC), correct=T) #correct: Yates'continuity correction. For reference only. 
# fisher.test(table(df$AD, df$RTQUIC)) # Expected count is <5 for one cell. 
# cramerV(table(df$AD, df$RTQUIC))
# phi(table(df$AD, df$RTQUIC), digits=6)


# #YOUNG-ONSET AD+: DISTRIBUTION
# table(YODdf$AD, YODdf$RTQUIC)

# #YOUNG-ONSET AD+: FISHER
# fisher.test(table(YODdf$AD, YODdf$RTQUIC)) # Expected count is <5 for one cell
# cramerV(table(YODdf$AD, YODdf$RTQUIC)) # Expected count is <5 for one cell
# phi(table(YODdf$AD, YODdf$RTQUIC), digits=6) # Expected count is <5 for one cell

# #YOUNG-ONSET AD+: DISTRIBUTION
# table(LODdf$AD, LODdf$RTQUIC)

# #LATE-ONSET AD+: FISHER
...
# boxplot(logabeta ~ RTQUIC, data= RTposdf, col = "white")$out #identify outliers: in AD positive df, none. 
# 	stripchart(logabeta ~ RTQUIC, data = RTposdf, method = "jitter", pch = 19, col = 2:4, vertical = TRUE, add = TRUE)
# boxplot(logabeta ~ RTQUIC, data= RTnegdf, col = "white")$out #create vector with outlier values (from AD-negative group)
# 	stripchart(logabeta ~ RTQUIC, data = RTnegdf, method = "jitter", pch = 19, col = 2:4, vertical = TRUE, add = TRUE)
...
# ggscatter(df, x = "NFL", y = "logabeta", add = "reg.line")+ 
# 	stat_regline_equation(aes()) 
# ggscatter(df, x = "NFL", y = "logabeta", color = "DX_APD", add = "reg.line")+ 
# 	stat_regline_equation(aes(color = DX_APD)) 
# ggscatter(df, x = "NFL", y = "logabeta", color = "RTQUIC", add = "reg.line")+
# 	stat_regline_equation(aes(color = RTQUIC))
# cor.test(df$logabeta, df$NFL) #corr
# summary(lm(df$logabeta ~ df$NFL)) #linear relationship 

# ## Inclusion of other variables as covariate:
# summary(lm(df$logabeta ~ df$LP2_Disease_Duration)) #no linear relationship
# summary(lm(df$logabeta ~ df$ttau)) #no linear relationship
# summary(lm(df$logabeta ~ df$ptau)) #no linear relationship

# # ABETA STATS: LINEAR REGRESSION MODEL
...
# df[5, c("ID", "abeta", "logabeta", "Onset", "RTQUIC", "NFL")] 
# df[21, c("ID", "abeta", "logabeta", "Onset", "RTQUIC", "NFL")] 
# df[27, c("ID", "abeta", "logabeta", "Onset", "RTQUIC", "NFL")] 
# df[37, c("ID", "abeta", "logabeta", "Onset", "RTQUIC", "NFL")] 
# df[45, c("ID", "abeta", "logabeta", "Onset", "RTQUIC", "NFL")] 
# df[65, c("ID", "abeta", "logabeta", "Onset", "RTQUIC", "NFL")] 
...
# df[(cooks.distance(stdmlr))>0.25, ]$ID #no subject >0.25
...
# 	# Add actual data
# 	geom_point(data=df, aes(x=Onset, y=logabeta, shape=DX_APD), size=4) + #Actual datapoints 
...
# cat("\n\n#######################################################################################################\n",
# 	"                                    8.1. ASYN-SAA+ & OTHER VARIABLES: BLR \n",
# 	   "#######################################################################################################\n")
...
# cat("\n\n#######################################################################################################\n",
# 	"                                    8.2. ASYN-SAA+ & OTHER VARIABLES: NFL \n",
# 	   "#######################################################################################################\n")
...
# cat("\n\n#######################################################################################################\n",
# 	"                                    8.3. ASYN-SAA+ & OTHER VARIABLES: CLINICAL \n",
# 	   "#######################################################################################################\n")
